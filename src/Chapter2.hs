...
subList start end (x : xs)
  | start < 0 || end - start < 0 = []
  | start > 0 = subList (start - 1) (end - 1) xs
  | otherwise = x : subList start (end - 1) xs
...
takeEven = takeEvenIndex 0
  where
    takeEvenIndex :: Int -> [a] -> [a]
    takeEvenIndex _ [] = []
    takeEvenIndex idx (x:xs) =
      if even idx
        then x : takeEvenIndex (idx+1) xs
        else takeEvenIndex (idx+1) xs
